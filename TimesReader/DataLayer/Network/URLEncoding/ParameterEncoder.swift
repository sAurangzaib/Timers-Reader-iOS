...
open class JSONParameterEncoder: ParameterEncoder {
    ...
    open func encode<Parameters: Encodable>(_ parameters: Parameters?,
        ...
        do {
            ...
            throw NSError(domain: "Encoding fail", code: 404, userInfo: [:])
        }
...
open class URLEncodedFormParameterEncoder: ParameterEncoder {
    ...
    open func encode<Parameters: Encodable>(_ parameters: Parameters?,
        ...
        guard let url = request.url else {
            throw NSError(domain: "Missing URL", code: 404, userInfo: [:])
        }

        guard let method = request.method else {
            let rawValue = request.method?.rawValue ?? "nil"
            throw NSError(domain: "Missing required component \(rawValue)", code: 404, userInfo: [:])
        }

        if destination.encodesParametersInURL(for: method),
           var components = URLComponents(url: url, resolvingAgainstBaseURL: false) {
            let query: String = try Result<String, Error> { try encoder.encode(parameters) }
                .mapError { _ in NSError(domain: "Encoding fail", code: 404, userInfo: [:]) }.get()
            let newQueryString = [components.percentEncodedQuery, query].compactMap { $0 }.joinedWithAmpersands()
            components.percentEncodedQuery = newQueryString.isEmpty ? nil : newQueryString

            guard let newURL = components.url else {
                throw NSError(domain: "Missing required component", code: 404, userInfo: [:])
            }
            ...
            .mapError { _ in NSError(domain: "Encoding failed", code: 404, userInfo: [:]) }.get()
        }
